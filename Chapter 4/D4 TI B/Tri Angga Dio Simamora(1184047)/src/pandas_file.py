...
def writePandas():
    buat_data = pandas.DataFrame({'name': ['Angga', 'Dinda'],
                                  'kelakuan': ['Kalem', 'Petacilen']})
    buat_data.to_csv('pandas_file_modipan.csv', index=False)

def openPandas():
    baca_data = pandas.read_csv('pandas_file_modipan.csv')
